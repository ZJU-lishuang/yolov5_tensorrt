...
static void lesson3(){
    ...
    for(auto& obj : objs){
        obj.left=obj.left-x;
        obj.top=obj.top-y;
        obj.right=obj.right-x;
        obj.bottom=obj.bottom-y;
        uint8_t b, g, r;
        std::tie(b, g, r) = random_color(obj.class_label);
        cv::rectangle(re, cv::Point(obj.left, obj.top), cv::Point(obj.right, obj.bottom), cv::Scalar(b, g, r), 5);

        auto name    = cocolabels[obj.class_label];
        auto caption = cv::format("%s %.2f", name, obj.confidence);
        int width    = cv::getTextSize(caption, 0, 1, 2, nullptr).width + 10;
        cv::rectangle(re, cv::Point(obj.left-3, obj.top-33), cv::Point(obj.left + width, obj.top), cv::Scalar(b, g, r), -1);
        cv::putText(re, caption, cv::Point(obj.left, obj.top-5), 0, 1, cv::Scalar::all(0), 2, 16);
    }

    printf("Save result to infer.jpg, %d objects\n", objs.size());
    cv::imwrite(cv::format("../images/render.jpg"), re);
